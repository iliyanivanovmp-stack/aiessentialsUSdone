# Write Blog Post from Google Ads Queue (with AI-Generated Images)

Write the next SEO-optimized blog post from the Google Ads publishing queue (`content/q-google-ads.md`) with 4 contextual AI-generated images. This is a fully automated command - one execution produces a complete blog post with images.
...
1. **content/q-google-ads.md** - Find the first item with `status: "queued"` - this is your topic
...
### Step 6: Update the Queue (q-google-ads.md)

Edit `content/q-google-ads.md` to update the published item:
...
```bash
git add content/blog-posts/[slug].md content/q-google-ads.md public/images/blog/ public/sitemap.xml public/llms.txt
git commit -m "Add blog post from Google Ads queue: [title] (with images)"
git push
```
